...
export default defineComponent({
  ...
  computed: {
    ...
    notesIndicator: function() {
      const availableNotesNames = [...new Set(this.availableNotes)].map((val: number) => noteNumberToName(val, this.track.song));
      const [lowLimit, highLimit] = this.track.getNotesLimits().map((val: number) => noteNumberToName(val, this.track.song));
      const notes = [];
  ...
  <div class="row">
      ...
      <div class="row">
        <div class="col-2">
          <div class="btn-group" role="group">
            <button @click="() => track.playpause()" class="btn btn-outline-primary playpause-track" title="Play/Pause track">
        ...
        <div class="col-4">
          <div class="input-group" role="group">
            ...
            <input class="form-control choose-track-base-velocity" type="number" min="0" max="100" :value="track.baseVelocity" @change="(ev: Event) => { track.load({ baseVelocity: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
          </div>
        </div>
        <div class="col-6">
      ...
      <div class="row">
        <div :class="track.notesMode === 'melotor' ? 'col-9' : 'col-6'">
          <div class="edit-track-notes input-group">
            <select class="input-group-text form-select" :value="track.notesMode" @change="(ev: Event) => track.applyNotesMode((ev.target as HTMLSelectElement).value as NotesMode, track.song)">
            ...
            <NumberListInput v-if="track.notesMode === 'manual'" :model-value="track.availableDegrees" @update:model-value="(val: number[]) => track.load({ availableDegrees: val })" />
            <NumberListInput v-if="track.notesMode === 'melotor' && track.melotor" readonly :model-value="track.melotor.currentMelo" />
        ...
        <div class="col-3" v-if="track.notesMode !== 'melotor'">
          <div class="input-group" role="group">
            <select class="form-select choose-track-play-mode" :value="track.playMode" @change="(ev: Event) => track.load({ playMode: (ev.target as HTMLSelectElement).value as any })">
            ...
            <select class="form-select choose-track-related-to" :value="track.relatedTo" @change="(ev: Event) => track.load({ relatedTo: (ev.target as HTMLSelectElement).value as any })">
              <option value="nil">----</option>
        ...
        <div class="col-3">
          <div class="input-group" role="group">
            ...
            <input class="form-control choose-track-gravity-center" type="number" min="1" max="127" :value="track.gravityCenter" @change="(ev: Event) => { track.load({ gravityCenter: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            <input class="form-control choose-track-gravity-strength" type="number" min="1" max="27" :value="track.gravityStrength" @change="(ev: Event) => { track.load({ gravityStrength: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
          </div>
        </div>
      </div>
    </div>
    <div class="col-12" v-if="computedView !== 'reduced' && track.notesMode === 'melotor' && track.melotor?.notesProbabilities !== undefined">
      <div class="row mt-1 mb-1">
        <MelotorEdit :melotor="track.melotor"
    ...
    <div class="col-12" v-if="computedView === 'expand'">
      <div class="row">
        <div class="col-12">
          <div class="edit-track-rythm input-group">
            <select class="input-group-text form-select" :value="track.rythmMode" @change="(ev: Event) => track.applyRythmMode((ev.target as HTMLSelectElement).value as RythmMode)">
              <option value="manual">Manual</option>
              <option value="preset">Preset</option>
            ...
            <NumberListInput class="w-50" v-if="track.rythmMode === 'manual' || track.rythmMode === 'durations'" :model-value="track.rythmDefinition" @update:model-value="(val: number[]) => { track.load({ rythmDefinition: val }); }" />
            ...
            <DivisionSelector :selected="track.division" @value-change="(val: number) => { track.load({ division: val }); }" />
          </div>
        </div>
      </div>
      <div class="col-12" v-if="computedView === 'expand'">
        <StepsSequencer v-if="track.rythmMode !== 'durations'"
      ...
      <div class="col-12" v-if="track.rythmMode === 'euclidean' && track.rythmDensity !== undefined && ShowEuclideanViews">
      ...
      <div class="col-12" v-if="computedView === 'expand'">
        <div class="input-group input-group">
          ...
          <input class="form-control" type="number" min="0" max="100" :value="track.proba" @change="(ev: Event) => { track.load({ proba: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
          <span class="input-group-text">V.Ampl.</span>
          <input class="form-control" type="number" min="0" :max="100 - (track.velCenter || 0)" :value="track.velAmplitude"  @change="(ev: Event) => { track.load({ velAmplitude: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
          <span class="input-group-text">V.Center</span>
          <input class="form-control" type="number" :min="track.velAmplitude || 0" :max="100 - (track.velAmplitude || 0)" :value="track.velCenter"  @change="(ev: Event) => { track.load({ velCenter: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
        </div>
      </div>
      <div class="col-12" v-if="computedView === 'expand'">
        <div class="row">
          <div class="col-2">
            <div class="input-group" role="group">
              ...
              <StrumSelector :selected="track.strumDelay" @change="(val: number) => { track.load({ strumDelay: val }); }" />
            </div>
          </div>
          <div class="col-2">
            <div class="input-group" role="group">
              <span class="input-group-text">Gate</span>
              <input class="form-control" type="number" min="0" max="100" :value="track.gate" @change="(ev: Event) => { track.load({ gate: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            </div>
          </div>
          <div class="col-2">
            <div class="edit-track-notes input-group">
              <span class="input-group-text">Octaves</span>
              <NumberListInput :model-value="track.octaves" @update:model-value="(val: number[]) => { track.load({ octaves: val }); }" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-9">
          <div class="input-group input-group">
            ...
            <input class="form-control choose-track-division" type="number" min="0" :value="track.division" @change="(ev: Event) => { track.load({ division: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            <span class="input-group-text">Density</span>
            <input class="form-control" type="number" min="0" max="64" :value="track.rythmDensity" @change="(ev: Event) => { track.load({ rythmDensity: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            <span class="input-group-text">Proba.</span>
            <input class="form-control" type="number" min="0" max="100" :value="track.proba" @change="(ev: Event) => { track.load({ proba: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            <span class="input-group-text">V.Ampl.</span>
            <input class="form-control" type="number" min="0" max="100" :value="track.velAmplitude" @change="(ev: Event) => { track.load({ velAmplitude: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
            <span class="input-group-text">V.Center</span>
          <input class="form-control" type="number" :min="track.velAmplitude || 0" :max="100 - (track.velAmplitude || 0)" :value="track.velCenter" @change="(ev: Event) => { track.load({ velCenter: parseInt((ev.target as HTMLInputElement).value, 10) }); }" />
          </div>
        </div>
