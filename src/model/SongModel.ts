...
export class SongModel {
  ...
  save(): SavedSongModel {
    ...
    return {
      ...dataToSave,
      tracks: Object.entries(this.tracks).reduce(
        (acc: Record<string, SavedTrackModel>, curr: [string, TrackModel]) => {
  ...
  tick() {
    ...
    if (this.clock % this.barLength === 0) {
      ...
      if (this.seqPlaying) {
        ...
        this.currentChord =
          this.chordProgression[this.position % this.chordProgression.length];
        if (this.currentChord !== previousChord) {
          changes.currentChord = this.currentChord;
        }
      }
      if (
        this.architecture === "clientserver" ||
        this.architecture === "back"
      ) {
  ...
  playpause(seq = true, tracks = true) {
    ...
    if (tracks) {
      ...
      if (
        ...
        this.callbacks?.remoteMessage("setSong", {
          tracksPlaying: this.tracksPlaying,
        });
      }
      for (const track of Object.values(this.tracks)) {
  ...
  rewind() {
    ...
    const targetChord =
      this.chordProgression[targetPosition % this.chordProgression.length];
    const changed =
      this.position !== targetPosition || this.currentChord !== targetChord;
    this.position = targetPosition;
    this.currentChord = targetChord;
    if (
      changed &&
      (this.architecture === "clientserver" || this.architecture === "back")
    ) {
      this.callbacks?.remoteMessage("setSong", {
        position: this.position,
...
export type SavedSongModel = Pick<
  SongModel,
  | "rootNote"
  | "scale"
  | "barLength"
