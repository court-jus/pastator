...
<script lang="ts">
import type { CallBacks, Tour, WsMessage } from "./model/types";
...
function loadMidiInputDeviceFromLocalStorage(
  midi: MIDIAccess,
  label: string
): MidiWrapper | undefined {
  const savedConfig = loadMidiDeviceFromLocalStorage(label) || "";
  if (!savedConfig) return undefined;
  return new MidiWrapper({
    midiDevice: midi.inputs.get(savedConfig),
    otherDevice: savedConfig,
  });
}

function loadMidiOutputDeviceFromLocalStorage(
  midi: MIDIAccess,
  label: string
): MidiWrapper | undefined {
  const savedConfig = loadMidiDeviceFromLocalStorage(label) || "";
  if (!savedConfig) return undefined;
  return new MidiWrapper({
    midiDevice: midi.outputs.get(savedConfig),
    otherDevice: savedConfig,
  });
}
...
export default {
        ...
        steps: [
          {
            ...
            content:
              "Hi, welcome to Pastator. This quick tour will guide you through the main features. If you finish or skip the tour and want to get back to it later, click the logo.",
          },
          {
            target: "#midi-clock-selection",
            content:
              "Start by selecting the MIDI device that will provide the clock. The logo's eyes should start blinking if MIDI clock events are received.",
          },
          {
            target: "#midi-out-selection",
            content:
              "Then select the MIDI device that will receive note events",
          },
        ],
        callbacks: {
    ...
    midiCallbacks(): CallBacks {
      return {
        playNote: (channel, note, velocity) =>
          this.midiOutputDevice &&
          this.midiOutputDevice.playNote(channel, note, velocity),
        stopNote: (channel, note) =>
          this.midiOutputDevice &&
          this.midiOutputDevice.stopNote(channel, note),
        remoteMessage: (messageType, messageData) => {
          this.remoteMessaging?.remoteMessage(messageType, messageData);
        },
      };
    },
  },
  mounted() {
    navigator
      ...
      .then(
        (access) => {
          ...
          this.midiClockDevice = loadMidiInputDeviceFromLocalStorage(
            access,
            "midiclock"
          );
          if (this.midiClockDevice)
            this.attachMidiLogger(this.midiClockDevice, "midiclock");
          this.midiOutputDevice = loadMidiOutputDeviceFromLocalStorage(
            access,
            "midioutput"
          );
          if (this.midiOutputDevice)
            this.attachMidiLogger(this.midiOutputDevice, "midioutput");
          this.midiCCDevice = loadMidiInputDeviceFromLocalStorage(
            access,
            "midicc"
          );
          if (this.midiCCDevice)
            this.attachMidiLogger(this.midiCCDevice, "midicc");
          this.midiNotesDevice = loadMidiInputDeviceFromLocalStorage(
            access,
            "midinotes"
          );
          if (this.midiNotesDevice)
            this.attachMidiLogger(this.midiNotesDevice, "midinotes");
    ...
    if (
      this.midiCCDevice &&
      this.midiCCDevice.isMidiInput() &&
      this.midiCCDevice.midiDevice
    ) {
    ...
    if (this.remoteMessaging === undefined) {
      ...
      if (params.wsUrl) {
        this.remoteMessaging = new MidiWrapper({
          otherDevice: "ws:remoteMessaging",
          wsUrl: params.wsUrl,
        });
        console.debug("Set up listeners from remoteMessaging");
        this.remoteMessaging.addEventListener(
          "setSong",
          (message: WsMessage) => {
  ...
  watch: {
    midiClockDevice(
      newDevice: MidiWrapper,
      oldDevice: MidiWrapper | undefined
    ) {
  ...
  methods: {
    addMidiLog(msg: string) {
      ...
      const [lastMessage] = this.midiLogs[this.midiLogs.length - 1];
      if (msg === lastMessage) {
...
<template>
  <nav
    class="navbar sticky-top navbar-light bg-light justify-content-between px-3 py-0"
  >
    <a class="navbar-brand pt-0 pb-0" href="#">
      <div
        class="logo-container clickable"
        @click="onLogoClicked"
        title="Click to launch the welcome tour."
      >
        <svg
          class="logo-led"
          width="36"
        ...
        >
          <circle
            ...
            :fill="
              song.clock
                ? song.clock % BarLength < BarLength / 2
                  ? 'green'
                  : 'red'
                : 'gray'
            "
          ...
          <path
            :fill="
              song.clock && song.clock % BarLength < BarLength / 4
                ? 'green'
                : 'transparent'
            "
          ...
          <path
            :fill="
              song.clock &&
              48 <= song.clock % BarLength &&
              song.clock % BarLength < (BarLength / 4) * 3
                ? 'green'
                : 'transparent'
            "
          ...
          <path
            :fill="
              song.clock && 72 <= song.clock % BarLength
                ? 'green'
                : 'transparent'
            "
    ...
    <ul class="nav nav-pills">
      <button
        ...
        @click="showMidiDevices = !showMidiDevices"
      >
        MIDI Devices
      </button>
    </ul>
  ...
  <div class="row">
    <div class="col-6 offset-6" v-if="midiSystem && showMidiDevices">
      <div>
        <div class="row" id="midi-clock-selection">
          <SelectMidiDevice
            :label="'Clock'"
            :modelValue="midiClockDevice"
            @update:modelValue="
              (newValue) => midiDeviceSelected(newValue, 'midiclock')
            "
            :midi="midiSystem"
          />
        </div>
        <div class="row" id="midi-out-selection">
          <SelectMidiDevice
            :label="'Output'"
            :modelValue="midiOutputDevice"
            @update:modelValue="
              (newValue) => midiDeviceSelected(newValue, 'midioutput')
            "
            :midi="midiSystem"
          />
        </div>
        <div
          class="row"
          id="midi-cc-in"
          v-if="midiOutputDevice && midiClockDevice"
        >
          <SelectMidiDevice
            :label="'Midi CC'"
            :modelValue="midiCCDevice"
            @update:modelValue="
              (newValue) => midiDeviceSelected(newValue, 'midicc')
            "
            :midi="midiSystem"
          />
        </div>
        ...
        >
          <SelectMidiDevice
            :label="'Midi Notes'"
            :modelValue="midiNotesDevice"
            @update:modelValue="
              (newValue) => midiDeviceSelected(newValue, 'midinotes')
            "
            :midi="midiSystem"
          />
        </div>
    ...
    <div class="col-12" v-if="midiDebug">
      <pre v-if="midiLogs">
        <span v-for="([log, count], idx) in midiLogs.slice().reverse().slice(0, 100)" v-bind:key="idx">{{count}} - {{ log }}</span>
      </pre>
    </div>
  </div>
  <v-tour
